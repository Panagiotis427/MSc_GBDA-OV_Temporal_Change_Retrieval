...
on Dynamic EarthNet (DEN), with two further datasets (QFabric and LEVIR-CC)
validating the dataset-agnostic design. The findings are reported under a strict statistical audit (random-ranking
...
$\approx$0.10. We trace the weak absolute signal to two fixable causes:
...
$\Delta$-scoring) recovers cross-validated mAP to $\approx$0.20 and makes
localised change-types --- new buildings and urban expansion, with new water
borderline --- retrievable above chance for the first time. On LEVIR-CC, where building and road changes are
salient and described in human language, the same frozen engine reaches
$\approx$0.55 mAP, showing the recovered signal scales with change salience. The
...
scored by one of three approaches (Section~\ref{sec:approaches}).
...
encoders, synthetic fixture, no network) passes 225 tests (full suite
241: 240 passed, 1 skipped) and covers the embeddings cache and round-trip,
...
(test 0.129) improves over RGB (0.050) but lags GeoRSCLIP NRG (0.426):
...
and on test it is worse than ProjectionHead PEFT ($0.041$). To check whether a
...
below frozen NRG zero-shot (0.426). This reinforces the project's core finding
...
cross-validated PEFT ($0.049 \pm 0.018$) is at or below zero-shot ---
confirming the in-distribution PEFT figures were memorisation.
...
$\approx$0.20 ceiling. A learned query-to-patch attention head was deliberately
...
is modest and PEFT still trails frozen NRG zero-shot (0.426), consistent with the
...
in genuine human language. LEVIR-CC pairs are parsed into open-vocabulary change
tags from their five captions (Section~\ref{sec:data}); three free-text queries
--- new buildings, a new road, buildings demolished --- are relevant to pairs
whose captions carry the matching tag. On the held-out test split (1929 pairs,
frozen encoders, RGB), Table~\ref{tab:levircc} reports macro mAP. The macro
random-ranking baseline is the mean query prevalence, 0.342 (building 45.2\%,
road 42.7\%, demolition 14.8\%).

\begin{table}[H]
    \centering
    \caption{LEVIR-CC open-vocabulary change retrieval (test split, 1929 pairs,
    frozen, RGB; macro mAP over three caption-grounded queries). Random-ranking
    baseline $\approx$0.342 (mean query prevalence). Best per row in bold.}
    \label{tab:levircc}
    \begin{tabular}{lcc}
        \toprule
        \textbf{Encoder} & \textbf{naive} & \textbf{zero-shot} \\
        \midrule
        GeoRSCLIP     & 0.540 & \textbf{0.557} \\
        CLIP ViT-L/14 & \textbf{0.543} & 0.498 \\
        RemoteCLIP    & 0.539 & \textbf{0.573} \\
        \bottomrule
    \end{tabular}
\end{table}

Retrieval is strong --- 0.50 to 0.57 macro mAP, roughly $+0.2$ above the 0.342
prevalence floor --- in sharp contrast to DEN, whose honest cross-validated
ceiling is $\approx$0.20. The difference is the nature of the change: LEVIR-CC's
new buildings and roads are large, high-contrast, and described in the same
vocabulary the CLIP text tower was trained on, so a frozen encoder localises them
readily, whereas DEN's directional spectral transitions are subtle and
AOI-specific. The RS-pre-trained encoders gain from the directional $\Delta$
(RemoteCLIP zero-shot 0.573 and GeoRSCLIP 0.557 both beat their naive scores),
while general CLIP prefers after-image content (naive 0.543 over zero-shot 0.498)
--- the same type-versus-transition split seen on QFabric
(Sections~\ref{sec:qfabric}, \ref{sec:qfabricstatus}). The lesson holds across all
three datasets: the open-vocabulary engine recovers the change signal in
proportion to its spatial and semantic salience --- strongly here, weakly on
DEN's subtle transitions.
...
\begin{table}[H]
    ...
    \begin{tabular}{lr}
        ...
        Fast test suite --- 225 tests, mock encoders, CPU & $\approx$65\,s \\
        \bottomrule
    \end{tabular}
\end{table}
...
a strict statistical audit. Four findings emerge. First, learned adapters only
memorise the training set: the large in-distribution PEFT lift
($0.043 \rightarrow 0.420$, up to 0.998 on QFabric) is the adapter scored on its
own training pairs, and cross-validated leakage-free PEFT ($0.049$) sits at or below
zero-shot ($0.100$), so low-compute fine-tuning is not shown to help retrieval
out-of-distribution. Second, out-of-distribution the frozen zero-shot wins, but
...
LoRA does not close the gap either (every rank/epoch overfits to test $\approx$0.07, far below frozen 0.426),
...
believed. Across the three datasets the recovered signal tracks change salience:
weak on DEN's subtle spectral transitions, dataset-dependent on QFabric, and
strong ($\approx$0.55 mAP) on LEVIR-CC's salient, human-captioned building change.
