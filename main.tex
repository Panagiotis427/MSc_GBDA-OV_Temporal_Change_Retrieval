...
across all three datasets: the open-vocabulary engine recovers the change signal
in proportion to its spatial and semantic salience --- strongly on salient
construction, weakly on subtle or sparse change.
...
detector. DEN carries no instance masks, so this metric is LEVIR-MCI-specific by
design.
...
within LEVIR-CC itself --- strong on salient building and road change (per-query
AP $\approx$0.6--0.8) yet near-random on subtle vegetation and sparse water.
