...
- **Status:** `src/lora_train.py`. LoRA was trained/evaluated on **Dynamic EarthNet** (QFabric used the ProjectionHead PEFT, §7.9); LEVIR-CC/MCI and SECOND-CC are **frozen-only**. Benchmarked: test mAP 0.071 (vs frozen zero-shot — single-split 0.426 / CV 0.100 ± 0.139, see REPORT.md §7.4 / Appendix B.8). In-app toggle: Settings → **Use LoRA embeddings** (requires pre-cached run). CLI: `--lora / --no-lora`.
