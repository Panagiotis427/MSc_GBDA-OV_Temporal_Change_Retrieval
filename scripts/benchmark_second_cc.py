...
``results/`` with per-query AP, like the LEVIR-CC / DEN / QFabric results.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="SECOND-CC open-vocab retrieval benchmark")
    ap.add_argument("--root", default="data/_second_cc/extracted/SECOND-CC-AUG",
                    help="SECOND-CC dir (SECOND-CC-AUG.json + {split}/rgb/{A,B})")
    ...
    ds = build_dataset("second_cc", root=args.root, split=args.split)
    pairs = ds.list_pairs()
    labels = [ds.get_pair_label(p) for p in pairs]
    n = len(pairs)
    queries = get_queries("second_cc")
    ...
    print(f"SECOND-CC {args.split}: {n} pairs | macro prevalence baseline = {macro_prev}")
    for q in queries:
    ...
    for enc_name in args.encoders:
        ...
        out = {"dataset": "second_cc", "encoder": enc_name, "split": args.split,
        ...
        op = Path(args.results_dir) / f"second_cc__{enc_name}__{args.split}.json"
