...
% Figures live under figures/; the NTUA logo (when added) under images/.
...
\title{
    ...
    % Supervisor filled from the Case 11 brief; verify before submission.
    Supervisor: \textit{Vasilis Tsironis} \par \vspace{0.5cm}
    Students: Panagiotis Giannikos, 03400245, panagiotisgiannikos@mail.ntua.gr \par
    % TODO: add co-author name, registration number, and e-mail (or delete this
    % line if this is a single-author submission).
    \textit{[Co-author name, registration number, e-mail]} \par \vspace{0.5cm}}
...
(VLMs)~\cite{clip}: instead of training a class-specific detector, we build a system that
...
confusion (``semantic drift''), and a Gradio~\cite{gradio} Semantic Change Search
Engine as the product deliverable. The primary dataset is Dynamic
EarthNet (DEN)~\cite{dynamicearthnet}, which provides daily multi-spectral Planet
Fusion~\cite{planetfusion} data ideal
for pinpointing the exact time-step of change; the architecture is kept open to
QFabric~\cite{qfabric}, LEVIR-CC~\cite{levircc}, and fMoW~\cite{fmow}.
...
ViT~\cite{vit} backbone, 768-d),
\texttt{georsclip} (\texttt{open\_clip}~\cite{openclip} ViT-B/32 + RS5M
checkpoint~\cite{georsclip}, 512-d), and
\texttt{remoteclip} (\texttt{open\_clip} ViT-L/14 + RemoteCLIP
checkpoint~\cite{remoteclip}, 768-d). The embedding dimension drives cache and
index sizing automatically.
...
land cover''). The loss is a masked symmetric InfoNCE~\cite{infonce}: pairs
...
\begin{figure}[H]
    ...
    the PEFT branch trains a small adapter $g$ over the change feature $\Delta f$;
    the naive and zero-shot scores require no training. The naive score ignores
    $T_1$ entirely, whereas the zero-shot $\Delta$-similarity measures how much more
    present the query becomes from $T_1$ to $T_2$.}
...
cross-validated against the official torchgeo~\cite{torchgeo} Dynamic EarthNet raster masks
...
QFabric~\cite{qfabric} is wired as a second dataset to validate the
dataset-agnostic design, using the polygon-centred crops from
\texttt{jirvin16/TEOChatlas}~\cite{teochat} with the
...
LEVIR-CC~\cite{levircc} adds a third dataset with a fundamentally different
...
LEVIR-MCI~\cite{levirmci} is a strict superset of LEVIR-CC --- the same
...
SECOND-CC~\cite{secondcc} pairs 6{,}041
bi-temporal scenes with 30{,}205 human change captions \emph{and} six-class
SECOND~\cite{second} semantic maps for both phases (ground, tree, low-vegetation, water,
...
and macro Recall@10 on the train split (Figures~\ref{fig:mapbars}
and~\ref{fig:recall} plot mAP and the full Recall@$K$ curves). This should be read as a sanity check, not
...
\begin{figure}[H]
    ...
    quantified in Sections~\ref{sec:crosssplit} and~\ref{sec:audit}.}
    \label{fig:mapbars}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cross_split__clip_vitl14__rgb.png}
        \caption{CLIP ViT-L/14.}
        \label{fig:crosssplit_clip}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cross_split__georsclip__rgb.png}
        \caption{GeoRSCLIP.}
        \label{fig:crosssplit_georsclip}
    \end{subfigure}\hfill
    \begin{subfigure}{0.32\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cross_split__remoteclip__rgb.png}
        \caption{RemoteCLIP.}
        \label{fig:crosssplit_remoteclip}
    \end{subfigure}
    \caption{Cross-split mAP per encoder (RGB): the PEFT train spike collapses on
    the held-out val/test splits for all three encoders --- the
    train-spike-then-collapse signature of train-AOI memorisation.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{color_ablation__zero_shot__train.png}
        \caption{train split.}
        \label{fig:colorablation_train}
    \end{subfigure}\hfill
    \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{color_ablation__zero_shot__test.png}
        \caption{held-out test split.}
        \label{fig:colorablation_test}
    \end{subfigure}
    \caption{Colour-mode ablation (zero-shot mAP). NRG slightly hurts in-distribution
    (\subref{fig:colorablation_train}) but dominates on held-out test
    (\subref{fig:colorablation_test}), most strongly for GeoRSCLIP --- though the
    absolute test lift is a fold artefact under cross-validation
...
LoRA~\cite{lora} applied to GeoRSCLIP's ViT-B/32 visual encoder targets the FFN projections
...
shuffled), with Benjamini--Hochberg~\cite{benjaminihochberg} FDR correction and leave-one-AOI-out
...
\subsection{Native 3\texorpdfstring{\,}{ }m raster source --- robustness of the data-source choice}
...
source: the native Planet-Fusion~\cite{planetfusion} surface-reflectance rasters
...
The replication above uses the caption-retrieval protocol. As an independent
second check, we also ran the native rasters through \emph{this repository's own}
cross-validated benchmark (\texttt{cv\_eval}: 5-fold \emph{AOI} partition,
dominant-class relevance over the ten DEN queries; Section~\ref{sec:audit}) --- the
same harness used for the JPEG subset, via the registry-resolved
\texttt{dynamic\_earthnet\_planet} loader. Table~\ref{tab:native3mcv} reports the
head-to-head. Under the identical encoder and metric the native 3\,m source
(zero-shot macro mAP $0.130\pm0.068$) sits \emph{at or above} the JPEG subset
($0.076$; $0.123$ under fraction relevance), despite covering fewer AOIs. The
comparison is not fully controlled --- the corpora differ in AOI count (23 vs 75)
and colour composite (native RGB vs JPEG NRG) --- but for this corpus colour barely
moves the score (georsclip: RGB $0.085$ vs NRG $0.100$), so the direction is robust:
the native source is no worse, corroborating the no-loss conclusion of
Section~\ref{sec:data} with the repository's native dominant-class metric rather than
the caption-retrieval one. The two remaining confounds are removed in the fully
controlled ablation below (Table~\ref{tab:jpegablation}, Figure~\ref{fig:jpegablation}).%
\footnote{The leakage-free $k$-fold PEFT estimate on the
native rasters is $0.215\pm0.306$; the large variance (one fold at $0.75$, the rest
$0.03$--$0.19$, driven by few positives per small fold) makes the zero-shot figure
the reliable point of comparison.}
...
isolate image fidelity itself we hold \emph{everything} fixed --- the same 23 AOIs,
253 pairs, RGB composite, encoder, zero-shot scoring and 5 AOI folds --- and vary
only the degradation applied to each tile before encoding. The native lossless
...
that changes between rows is the image. Two degradation families are swept: JPEG
...
\begin{figure}[H]
    ...
    curve is non-monotonic --- the most-compressed setting ($q{=}10$) scores
    \emph{above} the lightly-compressed ones --- so the variation is fold sampling,
    not a compression effect.}
    \label{fig:jpegablation}
\end{figure}

The reading is unambiguous: \emph{image fidelity is not the bottleneck}. The
largest gap from native across the entire JPEG sweep is $0.036$\,mAP --- about half
of one fold's standard deviation ($\sim$$0.065$) --- and the deltas alternate sign
with quality, so they are fold-sampling noise rather than a degradation signal. The
...
so we claim only that \emph{no degradation is detectable} at this corpus size, not
exact equivalence.} This is direct evidence for the report's central thesis
(Sections~\ref{sec:audit},~\ref{sec:nativeraster}): the ceiling is the frozen
...
on the \emph{sign} of the small downsampling deltas --- CLIP nominally rises toward
...
\subsection{Temporal pinpointing --- \texorpdfstring{\emph{when}}{when} does the change occur?}
\label{sec:temporalpinpoint}

The Case-11 brief singles out Dynamic EarthNet for its ability to ``pinpoint exact
...
time-steps''. The retrieval metrics so far answer \emph{which pair} shows a change
(ranking across AOIs); this is the orthogonal question of \emph{when} a change
happens (ranking time-steps within one AOI's timeline), and it is uniquely
accessible through the native loader's monthly raster stack. We build the loader
with monthly pairing so each AOI's consecutive pairs are its 24-month timeline; the
per-step change score is the same zero-shot $\Delta$-similarity used throughout
($\cos(q,f_t)-\cos(q,f_{t-1})$); a step is \emph{relevant} when the query predicate
fires on its monthly transition label. Within each AOI we rank the steps and score
the ranking by average precision (a per-AOI \emph{temporal} AP), and we report the
peak-hit rate --- whether the single highest-scoring step is a true transition, at
exact and $\pm1$-month tolerance. Significance uses a permutation test that shuffles
relevance \emph{within} each AOI (preserving its step count) plus BH-FDR.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Left:} per-query temporal mAP (within-AOI step ranking) against
    the permutation random floor (red ticks). \textbf{Right:} an example AOI
    timeline --- the zero-shot change score for \emph{``forest loss''} spikes at the
    labelled transition month (2019-07, dashed green), where the model's peak
    (orange star) also lands.}
    \label{fig:temporalpinpoint}
\end{figure}

The system does locate change in time above chance: macro temporal mAP
$0.309$ against a $0.207$ random floor, and within a $\pm1$-month tolerance the
model's peak step lands on a true transition $63\%$ of the time
...
(Section~\ref{sec:nativeraster}) the effect is encoder-dependent: repeating the
study with GeoRSCLIP (ViT-B/32) gives macro $0.146$ vs the same $0.208$ floor ---
\emph{at or below chance} --- so the larger general-purpose ViT-L/14 pinpoints
change in time where the smaller RS-pretrained backbone does not. The honest limit
is statistical power: only 1--7 AOIs carry positives per query, so although the
aggregate clears the floor and snow melt is individually significant at $p=0.025$,
no query survives BH-FDR --- the same small-labelled-AOI ceiling documented
throughout. Pinpointing \emph{when} a change occurs is feasible for salient,
abrupt transitions on a capable frozen encoder, but, like pinpointing \emph{where},
it is harder than retrieving \emph{whether} the change is present.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{qfabric_status_map__eval.png}
    \caption{QFabric status-transition retrieval (RQA5, RGB): the directional
    zero-shot score now matches or beats naive on every encoder, inverting the
    change-type regime of Figure~\ref{fig:qfabric_eval}. Both scores sit only just
    above the 0.043 macro-prevalence floor (dashed).}
    \label{fig:qfabricstatus}
\end{figure}
...
that deliverable was qualitative. LEVIR-MCI~\cite{levirmci} is a strict
...
LEVIR-CC's change is almost entirely building/road. SECOND-CC~\cite{secondcc}
is the breadth counterpart: 6{,}041 pairs with
...
\begin{figure}[H]
    ...
    memorisation reported in Section~\ref{sec:indist}.}
    \label{fig:confusion_train}
\end{figure}

\subsection{Direct seasonal-robustness probe --- stable-pair \texorpdfstring{$\Delta$}{Delta}-similarity gate}
...
concrete loader has yet to be added. Sentinel-1/2~\cite{sentinel2} fusion is likewise
