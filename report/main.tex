...
\title{
    ...
    Markos Syroukis, ------, ------@ntua.gr \par \vspace{0.5cm}}
...
\begin{figure}[H]
    ...
    show the engine's \emph{actual} top-1 retrieved pair --- Before, After, and the
...
The table above scores the native rasters under a 5-seed stratified AOI hold-out.
As a second, independent check under the repository's own metric, the native
rasters were also run through the cross-validated \texttt{cv\_eval} benchmark
(5-fold \emph{AOI} partition, dominant-class relevance over the ten DEN queries;
Section~\ref{sec:denmain}) --- the same harness used for the JPEG subset, via the
registry-resolved \texttt{dynamic\_earthnet\_planet} loader.
Table~\ref{tab:native3mcv} reports the head-to-head. Under the identical encoder
and metric the native 3\,m source (zero-shot macro mAP $0.130\pm0.068$) sits
\emph{at or above} the JPEG subset ($0.076$; $0.123$ under fraction relevance),
despite covering fewer AOIs. The comparison is not fully controlled --- the corpora
differ in AOI count (23 vs 75) and colour composite (native RGB vs JPEG NRG) --- but
for this corpus colour barely moves the score (georsclip: RGB $0.085$ vs NRG
$0.100$), so the direction is robust: the native source is no worse, corroborating
the no-loss conclusion of Section~\ref{sec:data} with the repository's native
dominant-class metric rather than the held-out replication above.\footnote{The
